...
function ManageRoles() {
  ...
  //     const grouping = availablePermissions.reduce((acc, curr) => {
  //       const split = curr.name.split("_");
  //       const group = split[1];
  //       if (acc[group]) {
  ...
  //     setGroupedPermissions(grouping);
        ...
        <table className="w-full">
          <tbody>
            ...
            <tr className="border">
              ...
              {groupedpermissions &&
                groupedpermissions["user"].map((role) => (
                  <td className="p-4 border" key={role.id}>
                    <input
                      type="checkbox"
                      name="user"
                      id={role.id}

...
function AppSummary() {
  ...
  const getUsersSummary = async () => {
    try {
      ...
      const getAllUserAveragemonthly = await api.get("/monitor-signups/all-users");
      const getAllAppUserAveragemonthly = await api.get("/monitor-signups/app-users");
      const getAllInternalUserAveragemonthly = await api.get("/monitor-signups/admins");
      ...
      if (getAveragecheckin.status === 200) {
        if (typeof getAveragecheckin.data === 'object' && getAveragecheckin.data !== null) {
      ...
      if (getAllUserAveragemonthly.status === 200) {
        if (typeof getAllUserAveragemonthly.data === 'object' && getAllUserAveragemonthly.data !== null) {
          setAllUserAveragemonthly(getAllUserAveragemonthly.data.growthRate || 0);
        } else {
      ...
      if (getAllAppUserAveragemonthly.status === 200) {
        if (typeof getAllAppUserAveragemonthly.data === 'object' && getAllAppUserAveragemonthly.data !== null) {
          setAllAppUserAveragemonthly(getAllAppUserAveragemonthly.data.growthRate || 0);
        } else {
      ...
      if (getAllInternalUserAveragemonthly.status === 200) {
        if (typeof getAllInternalUserAveragemonthly.data === 'object' && getAllInternalUserAveragemonthly.data !== null) {
          setAllInternalUserAveragemonthly(getAllInternalUserAveragemonthly.data.growthRate || 0);
        } else {
          setAllInternalUserAveragemonthly(getAllInternalUserAveragemonthly.data);
        }
  ...
  const renderPercentageChange = (value, timeframe) => {
    ...
    return (
      <div className={`flex items-center ${textColorClass} text-xs`}>
        <div className={`flex items-center justify-center ${bgColorClass} w-5 h-5 rounded-[5px] mr-2`}>
